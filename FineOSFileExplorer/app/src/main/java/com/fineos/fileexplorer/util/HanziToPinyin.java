...
public class HanziToPinyin {
   ...
   // private static final boolean DEBUG = Debug.isDebug();
    ...
    public static final byte[][] PINYINS = {
            ...
            { 67, 69, 0, 0, 0, 0 }, { 67, 69, 78, 0, 0, 0 },  { 90, 69, 78, 71, 0, 0 },
            ...
            { 68, 65, 0, 0, 0, 0 }, { 68, 65, 73, 0, 0, 0 }, { 83, 72, 65, 78, 0, 0 },
            { 68, 65, 78, 71, 0, 0 }, { 68, 65, 79, 0, 0, 0 }, { 68, 69, 0, 0, 0, 0 },
            { 68, 69, 73, 0, 0, 0 }, { 68, 69, 78, 0, 0, 0 }, { 68, 69, 78, 71, 0, 0 },
            { 90, 85, 79, 0, 0, 0 }, { 68, 73, 65, 0, 0, 0 }, { 68, 73, 65, 78, 0, 0 },
            ...
            { 74, 73, 65, 79, 0, 0 }, { 88, 73, 69, 0, 0, 0 }, { 74, 73, 78, 0, 0, 0 },
            ...
            { 76, 65, 79, 0, 0, 0 }, { 89, 65, 78, 0, 0, 0 }, { 76, 69, 73, 0, 0, 0 },
        ...
        token.type = Token.PINYIN;
			...
			final char tan = '\u8983';
				if (offset < 0 && tan == character) {
					...
					Log.d(TAG,"getToken tan return="+pinyin.toString());
					return token;
				}

	 final char shan = '\u5355';
          if (offset < 0 && shan == character) {
            offset = 295;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken shan return="+pinyin.toString());
            return token;
         }

	final char qiu = '\u4ec7';
         if (offset < 0 && qiu == character) {
            offset = 258;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken qiu return="+pinyin.toString());
            return token;
        }

	final char xie = '\u89e3';
         if (offset < 0 && xie == character) {
            offset = 355;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken xie return="+pinyin.toString());
            return token;
        }		

	final char zhai = '\u7fdf';
         if (offset < 0 && zhai == character) {
            offset = 390;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken zhai return="+pinyin.toString());
            return token;
        }

	final char zha = '\u67e5';
         if (offset < 0 && zha == character) {
            offset = 390;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken zha return="+pinyin.toString());
            return token;
        }	

	final char yue = '\u4e50';
         if (offset < 0 && yue == character) {
            offset = 370;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
	...
	final char po = '\u7e41';
         if (offset < 0 && po == character) {
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken po return="+pinyin.toString());
            return token;
        }

	final char miao = '\u7f2a';
         if (offset < 0 && miao == character) {
            offset = 205;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken miao return="+pinyin.toString());
            return token;
        }

         final char ou = '\u533a';
        if (offset < 0 && ou == character) {
            offset = 234;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken ou return="+pinyin.toString());
            return token;
        }

         final char shen = '\u6c88';
        if (offset < 0 && shen == character) {
            offset = 294;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken shen return="+pinyin.toString());
            return token;
        }		

        final char zhang = '\u957f';
        if (offset < 0 && zhang == character) {
            offset = 393;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("oyzw","getToken chang return="+pinyin.toString());
            return token;
        }
///FineOS start    hanzi shi
//add  by zhouguanghui  begin
    /*    final char don = '\u77f3';
        if (offset < 0 && don == character) {
            offset = 67;
            StringBuilder pinyin = new StringBuilder();
            for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
                pinyin.append((char)PINYINS[offset][j]);
            ...
            token.target = pinyin.toString();
            Log.d("zhouguanghui","getToken don return="+pinyin.toString());
            return token;
        }*/
//add  by zhouguanghui  end
///FineOS end
    ...
    public static String getPingYinFormString(String soucre) {
        StringBuilder pingying = new StringBuilder();
        ArrayList<Token> pingYings = HanziToPinyin.getInstance().get(soucre);
        for(Token token : pingYings) {
            pingying.append(token.target);
        }
        return pingying.toString();
    }
}
