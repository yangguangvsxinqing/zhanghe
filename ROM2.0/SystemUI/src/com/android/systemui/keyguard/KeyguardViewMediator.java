                ...
                case NETWORK_LOCKED:
                    synchronized (this) {
                        ...
                        if (KeyguardUtils.isAirplaneModeOn(mContext)) {
                            ...
                            mUpdateMonitor.setPinPukMeDismissFlagOfPhoneId(phoneId, false);
                        }
...
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT@{
        filter.addAction(HQ_HIDE_KEYGUARD) ;
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT@}
    ...
    public void onScreenTurnedOff(int why) {
        ...
        synchronized (this) {
            ...
            if (mExitSecureCallback != null) {
                ...
                Log.d(TAG, "onScreenTurnedOff() - call doKeyguardLaterLocked().") ;
    ...
    public void onDreamingStarted() {
        Log.d(TAG, "onDreamingStarted() is called.") ;
        synchronized (this) {
            Log.d(TAG, "onDreamingStarted() - mScreenOn = " + mScreenOn
                + " mLockPatternUtils.isSecure() = " + mLockPatternUtils.isSecure()) ;
            if (mScreenOn && mLockPatternUtils.isSecure()) {
                Log.d(TAG, "onDreamingStarted() - calls doKeyguardLaterLocked().") ;
                doKeyguardLaterLocked();
            }
        }
    }
    ...
    private void doKeyguardLocked(Bundle options) {
 	...
 	/*[FACTORY_TEST_BY_APK] modify start*/
        //added for disable lockscreen in factory kit mode. 2013.08.13. yjf.
        if(getFactoryKitMode() == 1)
        {
            Log.d(TAG, "doKeyguard: in factory kit mode, not showing lockscreen ");
            return ;
        }
        /*[FACTORY_TEST_BY_APK] modify end*/
        showLocked(options);
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// @}
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT @{
            else if (HQ_HIDE_KEYGUARD.equals(action)) {
                Log.w(TAG, "HQ_HIDE_KEYGUARD:    " + action);
                if (!isSecure()  && isShowing()){
                Log.w(TAG, "HQ_HIDE_KEYGUARD11111:    " + action);
...
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT @}
    ...
    private void playSound(int soundId) {
        if (soundId == 0) return;
        final ContentResolver cr = mContext.getContentResolver();
        if (Settings.System.getIntForUser(cr, Settings.System.LOCKSCREEN_SOUNDS_ENABLED, 1, UserHandle.USER_CURRENT) == 1) {
            ...
            Log.d(TAG, "playSound() - soundId = " + soundId
                + " mLockSoundVolume = " + mLockSoundVolume) ;
...
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT@{
    private static final String HQ_HIDE_KEYGUARD = "android.intent.action.HIDE_KEYGUARD";
//add by gongpeicai for gesture HQ_AP_GESTURE_SUPPORT@}
    ///M: add mIsIPOShutDown to fix ALPS01823479 issue.
    ...
    /*[FACTORY_TEST_BY_APK] modify start*/
    //added for factory kit. 2013.08.09.yjf.
    private int getFactoryKitMode()
    {
        ...
        File file = new File("/sys/devices/platform/hq_boot_status/boot_status");
        ...
        try {
            ...
            Log.w(TAG, "getFactoryKitMode:Failure reading /sys/devices/platform/hq_boot_status/boot_status:", e);
        }
        
        Log.w(TAG, "getFactoryKitMode:result:" + result);
