...
//    int preColor ;
    ...
    public static final String ACTION_THEME_APPLIED     = "com.android.server.ThemeManager.action.THEME_APPLIED";////fineos add by zhanghe
    ...
    private void recreateLabel() {
        ...
        if (mDual) {
           ...
           // mDualLabel.setTextColor(res.getColor(R.color.qs_tile_text));
            mDualLabel.setTextColor(res.getColor(R.color.qs_textcolor));
           ...
           // mLabel.setTextColor(res.getColor(R.color.qs_tile_text));
            mLabel.setTextColor(res.getColor(R.color.qs_textcolor));
  ...
  //fineos add by zhanghe begin
    protected void handleStateChanged2(QSTile.State state) {
        if (mIcon instanceof ImageView) {
            setIcon2((ImageView) mIcon, state);
        }
        if (mDual) {
            mDualLabel.setText(state.label);
            mDualLabel.setContentDescription(state.dualLabelContentDescription);
  ...
  //fineos add by zhanghe end
        ...
        if (!Objects.equals(state.icon, iv.getTag(R.id.qs_icon_tag))) {
			...
			//fineos add by zhanghe begin
				...
				if (state instanceof BooleanState) {
					booleanState = ((BooleanState) state).value;
				}
				boolean signalStateConnected = false;
				boolean fineosEnabled = false;
				if (state instanceof SignalState) {
					signalStateConnected = ((SignalState) state).connected;
					fineosEnabled = ((SignalState) state).fineosenabled;
				}

				Log.i("zhanghe", "booleanState=" + booleanState);
				Log.i("zhanghe", "signalStateConnected=" + signalStateConnected);
				
				int themeColor = ColorHelper.getColor(mContext.getResources(),
						R.color.systemui_qs_color);
				ColorFilter filter = new PorterDuffColorFilter(themeColor,
						PorterDuff.Mode.SRC_ATOP);
				int disableColor = R.color.systemui_qs_disableColor;
				ColorFilter disableColorfilter = new PorterDuffColorFilter(mContext
						.getResources().getColor(disableColor),
						PorterDuff.Mode.SRC_ATOP);
				
				if (booleanState) {					
					iv.setColorFilter(filter);// zhanghe
				}else if (signalStateConnected) {					
					iv.setColorFilter(filter);// zhanghe
				}else if (fineosEnabled) {				
				    iv.setColorFilter(filter);// zhanghe
			    }else {		
					iv.setColorFilter(disableColorfilter);// zhanghe
				}
				//fineos add by zhanghe end
        }
	...
	//fineos add by zhanghe begin
    protected void setIcon2(ImageView iv, QSTile.State state) {
//        if (!Objects.equals(state.icon, iv.getTag(R.id.qs_icon_tag))) {
            Drawable d = state.icon != null ? state.icon.getDrawable(mContext) : null;
            if (d != null && state.autoMirrorDrawable) {
                d.setAutoMirrored(true);
...
//            iv.setTag(R.id.qs_icon_tag, state.icon);
            if (d instanceof Animatable) {
				...
				ColorFilter disableColorfilter = new PorterDuffColorFilter(mContext
						.getResources().getColor(disableColor),
						PorterDuff.Mode.SRC_ATOP);
				Log.i("zhanghe", "themeColor = "+Integer.toHexString(themeColor));
				Log.i("zhanghe", "preColor = "+preColor);
				if(preColor != Integer.toHexString(mContext.getResources().getColor(disableColor))){
					preColor = Integer.toHexString(themeColor);
					Log.i("zhanghe", "hahaha");
  ...
  //fineos add by zhanghe end
    public void onStateChanged(QSTile.State state) {
      ...
      //fineos add by zhanghe begin
        mHandler.obtainMessage(H.STATE_CHANGED_2, state).sendToTarget();
      //fineos add by zhanghe end  
    }
    ...
    private class H extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == STATE_CHANGED) {
                ...
                state2 = (State) msg.obj;////fineos add by zhanghe 
            }
          //fineos add by zhanghe begin
