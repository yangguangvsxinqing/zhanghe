...
///FineOS Notificaiton qs start
		...
		{
		        return false; ///FineOS Notificaiton qs 
		}else{
		        return true;
		}
///FineOS Notificaiton qs end
...
///FineOS Notificaiton qs start
    ...
    protected void handleLongClick() {
       // Intent intent = new Intent(BLUETOOTH_SETTINGS);
...
///FineOS Notificaiton qs start	
		...
		if (enabled) {
            ...
            if (connected) {
                ...
                state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_bluetooth_connecting);
                state.contentDescription = mContext.getString(
                        R.string.accessibility_quick_settings_bluetooth_connecting);
                state.label = mContext.getString(R.string.quick_settings_bluetooth_label);
            } else {
                state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_bluetooth_on);
                state.contentDescription = mContext.getString(
            ...
            state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_bluetooth_off);
            state.label = mContext.getString(R.string.quick_settings_bluetooth_label);
            state.contentDescription = mContext.getString(
...
///FineOS Notificaiton qs start	
		  if(SystemProperties.get("ro.fineos.framework", "no").equals("yes")){
                    item.icon = R.drawable.fineos_ic_qs_bluetooth_on;
                    item.line1 = device.name;
                    if (device.state == PairedDevice.STATE_CONNECTED) {
                        item.icon = R.drawable.fineos_ic_qs_bluetooth_connected;
                        item.line2 = mContext.getString(R.string.quick_settings_connected);
                        item.canDisconnect = true;
                    } else if (device.state == PairedDevice.STATE_CONNECTING) {
                        item.icon = R.drawable.fineos_ic_qs_bluetooth_connecting;
