...
///FineOS Notificaiton qs end
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        state.fineosenabled = state.enabled;//add by zhanghe 
...
///FineOS Notificaiton qs start
        if(SystemProperties.get("ro.fineos.framework", "no").equals("yes")){
        if (!state.enabled) {
            state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_wifi_disabled);
            state.label = r.getString(R.string.quick_settings_wifi_label);
            signalContentDescription = r.getString(R.string.accessibility_wifi_off);
        } else if (wifiConnected) {
            state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_wifi_enable);
            state.label = removeDoubleQuotes(cb.enabledDesc);
            signalContentDescription = cb.wifiSignalContentDescription;
        } else if (wifiNotConnected) {
            state.icon = ResourceIcon.get(R.drawable.fineos_ic_qs_wifi_0);
            state.label = r.getString(R.string.quick_settings_wifi_label);
