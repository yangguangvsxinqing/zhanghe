...
// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos
    ...
    /** Focuses the task at the specified index in the stack */
    void dismissForntTask(int taskIndex, boolean scrollToNewPosition, final boolean animateFocusedState) {
        // Return early if the task is already focused
        if (taskIndex == mFocusedTaskIndex) return;
...
// fineos add start
		...
		//private final static int DELAY_TIME = 330;
			...
			private final static int DISMISS = 1;
			 ...
			 public void handleMessage(Message msg) {		
				 ...
				 case REMOVE:
						mAllTaskCount--;
						resetFocusedTask();
						dismissForntTask(mAllTaskCount,true,false);
						mClearTaskCount++;
						if(mClearTaskCount >= mNumTasks){
							removeTask();
	...
	public void removeTask(){
		int stackCount = mStack.getTaskCount();
		for(int i = 0; i < stackCount; i++){
			Task t = mStack.getTasks().get(i);
		// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
			if(isLockApp(t.key.baseIntent.getComponent().getPackageName()))
				continue;
...
// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
	...
	public void dismissAllTaskView() {
		...
		mAllTaskCount = mStack.getTaskCount();
		if(mAllTaskCount <= 0)
			return;
		...
		resetFocusedTask();
		setForntFocusTask(mAllTaskCount - 1,true,false);

		myHandler = new MyHandler(); 
		
		mNumTasks = mStack.getTaskCount();
		int c=0;
		...
		for(int i = 0; i < mNumTasks; i++){
		   ...
		   message.what = MyHandler.REMOVE;
		   c++;
		   myHandler.sendMessageDelayed(message, DELAY_TIME * c);	
		}

    }

	public int getTaskCount() {
		int count = mStack.getTaskCount();
		return count;
    }
...
// fineos add end
    ...
    if(SystemProperties.get("ro.fineos.framework", "no").equals("yes")){
        return (TaskView) mInflater.inflate(R.layout.fineos_recents_task_view, this, false);
    }else{
