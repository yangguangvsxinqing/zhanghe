...
// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos end
    ...
    public SwipeHelper(int swipeDirection, Callback callback, float densityScale,
            float pagingTouchSlop, Context context) {
        mCallback = callback;
		...
		// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
		mySharedPreferences = context.getSharedPreferences(PREFERENCES_RECENT_NAME, Activity.MODE_PRIVATE); 
    ...
    private void endSwipe(VelocityTracker velocityTracker) {
        ...
        if (dismissChild && translation > 0 ){//&& (!childSwipedFarEnough || translation > 0)) {
            // flingadingy
            ...
            mCallback.onDragCancelled(mCurrView);
            snapChild(mCurrView, velocity);   
        	 isLockApplication(baseIntent.getComponent().getPackageName());
			 tv.reloadHeaderView();
        }
		// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos end
	...
	// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
	public void isLockApplication(String packageName ){
			Log.e("daipan","[isLockApplication][packageName]" + packageName);
			
			Boolean isLocked = mySharedPreferences.getBoolean(packageName, false);		
