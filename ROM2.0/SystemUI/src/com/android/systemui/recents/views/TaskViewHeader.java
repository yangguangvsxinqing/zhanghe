...
// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
 	ImageView mApplicationLockIcon;
...
// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos end
    ...
    protected void onFinishInflate() {
		...
		// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
		mApplicationLockIcon = (ImageView) findViewById(R.id.application_lock_icon);
		// FINEOS_REMOVE_ALLTASK_RECENTS add by fineos end
    ...
    public void rebindToTask(Task t) {
		...
		final ResolveInfo resolveInfo = pm.resolveActivity(t.key.baseIntent, 0);

		int iconId = resolveInfo.activityInfo.getIconResource();


        Drawable icon = ThemeHelper.getDrawable(pm, t.key.baseIntent.getComponent().getPackageName(),
                           iconId,  resolveInfo.activityInfo.applicationInfo, resolveInfo.activityInfo.name);
		if(icon != null){
			mApplicationIcon.setImageDrawable(icon);
       }else if (t.activityIcon != null) {
	   ...
	   // FINEOS_REMOVE_ALLTASK_RECENTS add by fineos start
		if(isLock(t.key.baseIntent.getComponent().getPackageName())){

			if(mApplicationLockIcon != null)
				mApplicationLockIcon.setVisibility(View.VISIBLE);
		}else{
			if(mApplicationLockIcon != null)
				mApplicationLockIcon.setVisibility(View.INVISIBLE);
		}
      // FINEOS_REMOVE_ALLTASK_RECENTS add by fineos end
...
public boolean isLock(String packageName){
		...
		SharedPreferences mySharedPreferences = mContext.getSharedPreferences(PREFERENCES_RECENT_NAME, Activity.MODE_PRIVATE);
		boolean isLock = mySharedPreferences.getBoolean(packageName, false);
