...
public class NotificationPanelView extends PanelView implements
	...
	/// M:[HQ_BOWAY_SYSTEMUI] show Boway keyguard{@
    ...
    protected void onFinishInflate() {
		...
		if (FeatureOption.HQ_BOWAY_SYSTEMUI){
			mIvUserAvatar = (ImageView) mHeader.findViewById(R.id.multi_user_avatar);
    ...
    private void setQsExpanded(boolean expanded) {
		...
		/// M:[HQ_BOWAY_KEYGUARD] show Boway keyguard{@
		if (FeatureOption.HQ_BOWAY_KEYGUARD && mKeyguardShowing){
			return;
		}
		/// @}
        if (changed) {
    ...
    public void setBarState(int statusBarState, boolean keyguardFadingAway,
       ...
       /// xuqian modified begin , hide BottomArea and statusview
	   FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
	   if(fineUtilities.useFineOSlideKeyguard()){   //xuqian
        	 
    	   setKeyguardStatusViewVisibility(StatusBarState.SHADE, true, false);
           setKeyguardBottomAreaVisibility(StatusBarState.SHADE, false);
        ...
        /// xuqian modified end , hide BottomArea and statusview
    ...
    private void updateQsState() {
    	 ...
    	 //FineOS xuqian add begin hide keyguard view  when keyguard not show
		 
		 boolean isKeyguardShowing = StatusBarKeyguardViewManager.isKeyguardShowing();
    	 Log.v(TAG,"isKeyguardShowing:"+mKeyguardShowing);
    	 FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
    	 if(fineUtilities.useFineOSlideKeyguard()){           //xuqian add
		 
		 View keyguard = this.getChildAt(KEYGUARD_INDEX);
    	 Log.v(TAG,"updateQsState(),mKeyguardShowing = "+mKeyguardShowing+", keyguard = "+keyguard+" ,mQsExpanded :"+mQsExpanded +",mQsFullyExpanded"+mQsFullyExpanded);
    	 if(mKeyguardShowing){
    		 if(keyguard!=null){
    			 keyguard.setVisibility(View.VISIBLE);
    		 }
		   Log.v(TAG,"mQsFullyExpanded :"+mQsFullyExpanded);	 
    	   if(mQsFullyExpanded){
	  ...
	  setBackgroundColor(0xff000000);	
    		 ...
    		 if(keyguard!=null&& (keyguard instanceof KeyguardHostView)){
    			 keyguard.setVisibility(View.GONE);
    		 }

            setBackgroundColor(0);	 
    		 
    	 }  // end of mKeyguardShowing
    ...
    private void flingSettings(float vel, boolean expand, final Runnable onFinishRunnable) {
		/// M:[HQ_BOWAY_SYSTEMUI] show Boway keyguard{@
		if (FeatureOption.HQ_BOWAY_SYSTEMUI && mIvUserAvatar != null){
			if (expand){
				mIvUserAvatar.setImageResource(R.drawable.ic_sysbar_setting);
			} else {
			    mIvUserAvatar.setImageResource(R.drawable.ic_sysbar_notification);
			}
		}
		///@}
    ...
    protected boolean isScrolledToBottom() {
		...
		///FineOS xuqian add starts  to disable up gesture when keyguard showing
    	 Log.v(TAG,"mQsExpanded: "+mQsExpanded);
    	 boolean isKeyguardShowing = StatusBarKeyguardViewManager.isKeyguardShowing();
    	 FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
        if(fineUtilities.useFineOSlideKeyguard()&&isKeyguardShowing){           //xuqian add
    		 return false ;
    	 }
    	 ///FineOS add end 
        if (!isInSettings()) {
    ...
    public void onClick(View v) {
		...
		/// M:[HQ_BOWAY_SYSTEMUI] show Boway keyguard{@
		if (FeatureOption.HQ_BOWAY_SYSTEMUI){
		    if (v.getId() == R.id.multi_user_avatar){
				onQsExpansionStarted();
	            if (mQsExpanded) {
	                flingSettings(0 /* vel */, false /* expand */);
	            } else if (mQsExpansionEnabled) {
