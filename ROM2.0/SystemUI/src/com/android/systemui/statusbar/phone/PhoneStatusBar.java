...
import com.fineos.keyguard.FileLoader;
...
/// M:[HQ_BOWAY_KEYGUARD] show Boway keyguard{@
import com.android.systemui.statusbar.fifthlock.KeyguardBottomAreaViewHQ;
import com.huaqin.common.featureoption.FeatureOption;
///@}
/*[HQ_NavigationBar_HIDE] modify start*/
///FineOS Notification head start
import com.android.systemui.statusbar.policy.MyInfoControllerFineOS;
...
//fineos SCREEN_RECORDER_SUPPORT start
import java.io.IOException;
import android.content.pm.PackageManager;

import java.text.SimpleDateFormat;
import java.util.Date;
//fineos SCREEN_RECORDER_SUPPORT end

///M:[HQ_BOWAY_SYSTEMUI] modify{@
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
        DragDownHelper.DragDownCallback, ActivityStarter, OnUnlockMethodChangedListener,
        NotificationStackScrollLayout.OnDismissStateChangeListener{
    ///@}
...
///FineOS Notification head start  	
    MyInfoControllerFineOS mMyInfoController;
///FineOS Notification head end  		
    ...
    /// M [HQ_BOWAY_KEYGUARD] show Boway keyguard{@
    KeyguardBottomAreaViewHQ mKeyguardBottomAreaViewHQ;
    /// @}    
    ...
    /*[HQ_NavigationBar_HIDE] modify start*/
    private HideNavigationbarBroadcastReceiver mReceiver = null;  
    private int testResult = 1;  
    private ImageButton mPortraitButton,mLandButton;
     private class HideNavigationbarBroadcastReceiver extends BroadcastReceiver {
    	@Override
    	public void onReceive(Context context, Intent intent) {
    		if ("broad.nabigationbar.show".equals(intent.getAction())) {
                 testResult = intent.getIntExtra("testResult",1);
    		}else if("broad.navigationbar.dimiss".equals(intent.getAction())){
    		 testResult = intent.getIntExtra("testResult",1);
    		}
               if(mNavigationBarView == null){
                ...
                (NavigationBarView) View.inflate(mContext, R.layout.navigation_bar, null);    
                     ...
                     mPortraitButton = (ImageButton) mNavigationBarView.findViewById(R.id.hide);
                     mLandButton = (ImageButton) mNavigationBarView.findViewById(R.id.hide_land);
                   if(testResult == 0){
                   mPortraitButton.setVisibility(View.VISIBLE);
                   mLandButton.setVisibility(View.VISIBLE);
                   prepareNavigationBarView();
                   }else{
                   mPortraitButton.setVisibility(View.GONE);
		...
		filter.addAction("broad.nabigationbar.show");
		filter.addAction("broad.navigationbar.dimiss");
        mReceiver = new HideNavigationbarBroadcastReceiver();
		mContext.registerReceiver(mReceiver, filter);

	}
    /*[HQ_NavigationBar_HIDE] modify end*/
...
/*[HQ_NavigationBar_HIDE] modify end*/
	...
	//fineos STATUSBAR_ICON_COLOR_SUPPORT start
	registerIconColorBroadcast();
	//fineos STATUSBAR_ICON_COLOR_SUPPORT end
	
	//fineos SCREEN_RECORDER_SUPPORT start
	registerScreenRecorderBroadcast();
	//fineos SCREEN_RECORDER_SUPPORT end
        ...
        if (FeatureOption.HQ_BOWAY_SYSTEMUI){
			ViewStub viewStubClearAll = (ViewStub) mStatusBarWindow.findViewById(R.id.keyguard_vstub_clearall);
            viewStubClearAll.inflate();
			mDismissButton=(Button) mStatusBarWindow.findViewById(
	                R.id.dismiss_button_test);
	        mStackScroller.setDismissStateChangeListener(this);
		...
		///FineOS Notification head start  			
        mMyInfoController = new MyInfoControllerFineOS(mContext);		
	///FineOS Notification head end  
	...
	/// M:[HQ_BOWAY_KEYGUARD] show Boway keyguard{@
		if (FeatureOption.HQ_BOWAY_KEYGUARD){
			FrameLayout frameSystemKeyguardBottom = (FrameLayout) mStatusBarWindow.findViewById(R.id.keyguard_flayout_area);
			frameSystemKeyguardBottom.setVisibility(View.GONE);
			ViewStub viewStub = (ViewStub) mStatusBarWindow.findViewById(R.id.keyguard_vstub_bottom);
            viewStub.inflate();
			mKeyguardBottomAreaViewHQ = 
                (KeyguardBottomAreaViewHQ) mStatusBarWindow.findViewById(R.id.keyguard_bottom_lock);
		    mKeyguardBottomAreaViewHQ.setActivityStarter(this);
		    mKeyguardBottomAreaViewHQ.setFlashlightController(mFlashlightController);
		}
    	///@}        
...
///FineOS Notification head start  	
        ...
        mHeader.setMyInfoController(mMyInfoController);
        mKeyguardStatusBar.setMyInfoController(mMyInfoController);
        mMyInfoController.reloadMyInfo();
///FineOS Notification head end   	
    ...
    private View.OnClickListener mlandHideClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
           removeNavigationBar();
        }
    ...
    private void prepareNavigationBarView(boolean forceReset) {
        ...
        mNavigationBarView.getHideButton().setOnClickListener(mhideClickListener); } else{
        mNavigationBarView.getLandHideButton().setOnClickListener(mlandHideClickListener);}
        ...
        if (forceReset) {
           mNavigationBarMode = 0;
            ...
            topAppWindowChanged(false);//mShowMenu
        }
  ...
  /*[HQ_NavigationBar_HIDE] modify end*/
    ...
    private void prepareNavigationBarView() {
        ...
        /*[HQ_NavigationBar_HIDE] modify start*/
        if (mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT){
              mNavigationBarView.getHideButton().setOnClickListener(mhideClickListener);
        } else{
              mNavigationBarView.getLandHideButton().setOnClickListener(mlandHideClickListener);
        }
        /*[HQ_NavigationBar_HIDE] modify end*/
    ...
    public void refreshAllStatusBarIcons() {
        ...
        //fineos STATUSBAR_ICON_COLOR_SUPPORT start
        refreshAllStatusBarIconsColorFilter();
        //fineos STATUSBAR_ICON_COLOR_SUPPORT end
    }
    ...
    private void refreshAllIconsForLayout(LinearLayout ll) {
        ...
        for (int n = 0; n < count; n++) {
            ...
            if (child instanceof StatusBarIconView) {
                //fineos STATUSBAR_ICON_COLOR_SUPPORT start
                ColorFilter cf = ((StatusBarIconView) child).getColorFilter();
                if(cf != null){
                    ((StatusBarIconView) child).setColorFilter(getIconColor());
                }
                //fineos STATUSBAR_ICON_COLOR_SUPPORT end
    ...
    public void updateMediaMetaData(boolean metaDataChanged) {
		...
		/// xuqian modified begin , do not show album on lockscreen  when music on
       FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
		if(fineUtilities.useFineOSlideKeyguard()){
		
		   artworkBitmap = null ;   ///do not show music album as lockscreen bg  
		...
		/// xuqian modified end , do not show album on lockscreen  when music on
        ...
        public void handleMessage(Message m) {
            ...
            switch (m.what) {
                ...
                case MSG_LAUNCH_TRANSITION_TIMEOUT:
                    ...
                    break;
                //fineos STATUSBAR_ICON_COLOR_SUPPORT start
                case MSG_UPDATE_ICON_COLOR:
                    updateIconColor();
			removeMessages(MSG_UPDATE_ICON_COLOR);
                    break;
                //fineos STATUSBAR_ICON_COLOR_SUPPORT end
            }
        }
    ...
    public void topAppWindowChanged(boolean showMenu) {
        ...
        //fineos STATUSBAR_ICON_COLOR_SUPPORT start
        //mHandler.removeMessages(MSG_UPDATE_ICON_COLOR);
        //mHandler.sendEmptyMessageDelayed(MSG_UPDATE_ICON_COLOR, 100);
        //fineos STATUSBAR_ICON_COLOR_SUPPORT end
    ...
    protected void dismissKeyguardThenExecute(final OnDismissAction action,
		...
		Log.v(TAG,"dismissKeyguardThenExecute,mStatusBarKeyguardViewManager.isShowing() :"+mStatusBarKeyguardViewManager.isShowing());	
        if (mStatusBarKeyguardViewManager.isShowing()) {

        	FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);     //xuqian modified ,go to setting enable
           if(fineUtilities.useFineOSlideKeyguard()){   //xuqian
             action.onDismiss();
           }else{
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    public int getBarState() {
	 ...
	 private void setBackground(){
		...
		Log.v(TAG,"setBackground,mFileLoader="+mFileLoader+",mHolder: "+mHolder);
		if(mFileLoader!=null){	
			
					
			mFileLoader.setLockWallpaperFromTheme(mHolder);
			mHolder.setAlpha(0.0f);
			mHolder.animate()
            .alpha(1.0f)
            .setInterpolator(mBackdropInterpolator)
            .setDuration(2)
    ...
    public void showKeyguard() {
        ...
        FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
        Log.v(TAG,"PhoneStatusBar,showKeyguard()...");
		if(fineUtilities.useFineOSlideKeyguardNew()){
		   setBarState(StatusBarState.SHADE_LOCKED); //xuqian
		}else{
	...
	Log.v(TAG,"hasArtWork :"+hasArtWork +",mState:"+mState);
	
	if (SystemProperties.getBoolean("ro.fineos.framework", false)&&!hasArtWork&&(mState == StatusBarState.KEYGUARD || mState == StatusBarState.SHADE_LOCKED)){
	...
	if (!fineUtilities.useFineOSlideKeyguard()){
	   Log.v(TAG,"mKeyguardStatusView: "+mKeyguardStatusView);
	   ((KeyguardStatusView)mKeyguardStatusView).setMyinfo();
	}
    ...
    private void updateDozingState() {
        if (mState != StatusBarState.KEYGUARD && !mNotificationPanel.isDozing()) {
			Log.d("BingWu.Lee", "updateDozingState return");
            return;
        }
    ...
    public void goToKeyguard() {
       ...
       FineOSUtilities fineUtilities = FineOSUtilities.getInstance(mContext);
       Log.v(TAG,"PhoneStatusBar,goToKeyguard()...");
		if(fineUtilities.useFineOSlideKeyguardNew() ){
		   setBarState(StatusBarState.SHADE_LOCKED); //xuqian
		}else{
		   setBarState(StatusBarState.KEYGUARD);
   ...
   ///@}
       //fineos STATUSBAR_ICON_COLOR_SUPPORT start
       private static final int ICON_COLOR_DARK = 0xff757575;
       private static final int ICON_COLOR_LIGHT = 0xffffffff;
	private static int mIconColor = ICON_COLOR_LIGHT;
	...
	private int getPackageIconColor(String packageName)  
	{  
		int color = ICON_COLOR_LIGHT;
		if(packageName != null){
			if(packageName.equals("com.android.mms") ||
				packageName.equals("com.android.yepvieoplayer") ||
				packageName.equals("com.fineos.theme") ||
				packageName.equals("com.fineos.calculator") ||
				packageName.equals("com.fineos.fileexplorer") ||
				packageName.equals("com.fineos.gallery3d")){
	...
	public void updateIconColor(){
		...
		int color = getPackageIconColor(packageName);
		if(color == mIconColor){
			return;
		}
		mIconColor = color;
		*/
		...
		TextView clock = (TextView) mStatusBarView.findViewById(R.id.clock);
		if(clock != null){
			clock.setTextColor(getIconColor());
		}
		BatteryMeterView battery = (BatteryMeterView) mStatusBarView.findViewById(R.id.battery);
		if(battery != null){
			battery.setIconColor(getIconColor());
		}
		if(mKeyguardStatusBar != null){
			BatteryMeterView batteryKeyguard = (BatteryMeterView) mKeyguardStatusBar.findViewById(R.id.battery);
			if(batteryKeyguard != null){
				batteryKeyguard.setIconColor(ICON_COLOR_LIGHT);
			}
		}
		
		SignalClusterView signalCluster = (SignalClusterView) mStatusBarView.findViewById(R.id.signal_cluster);
		if(signalCluster != null){
			signalCluster.setIconColor(getIconColor());
		}
		if(mKeyguardStatusBar != null){
			SignalClusterView signalClusterKeyguard = (SignalClusterView) mKeyguardStatusBar.findViewById(R.id.signal_cluster);
			if(signalClusterKeyguard != null){
				//signalClusterKeyguard.setIconColor(getIconColor());
			}
		}
		if(mTicker != null){
			mTicker.setIconColor(getIconColor());
		}
		if(mKeyguardStatusBar != null){
			//mKeyguardStatusBar.setIconColor(getIconColor());
		}
	}
	
	private IconColorBroadcastReceiver mIconColorReceiver = null;  
	private class IconColorBroadcastReceiver extends BroadcastReceiver {
		@Override
		public void onReceive(Context context, Intent intent) {
			if ("android.fineos.action.UPDATE_STATUSBAR_ICON_COLOR".equals(intent.getAction())) {
				int bgColor = intent.getIntExtra("bg_color",0);
				boolean bgDark = isDarkColor(bgColor);
				if(bgDark){
					mIconColor = ICON_COLOR_LIGHT;
				}else{
					mIconColor = ICON_COLOR_DARK;
				}
				mHandler.removeMessages(MSG_UPDATE_ICON_COLOR);
				mHandler.sendEmptyMessageDelayed(MSG_UPDATE_ICON_COLOR, 100);
			}
	...
	private void registerIconColorBroadcast() {
		IntentFilter filter = new IntentFilter();
		filter = new IntentFilter();
		filter.addAction("android.fineos.action.UPDATE_STATUSBAR_ICON_COLOR");
		mIconColorReceiver = new IconColorBroadcastReceiver();
		mContext.registerReceiver(mIconColorReceiver, filter);
	}
	...
	public void refreshAllStatusBarIconsColorFilter() {
		refreshAllIconsForLayoutColorFilter(mStatusIcons);
	}

	private void refreshAllIconsForLayoutColorFilter(LinearLayout ll) {
		final int count = ll.getChildCount();
		for (int n = 0; n < count; n++) {
			View child = ll.getChildAt(n);
			if (child instanceof StatusBarIconView) {
	...
	//fineos SCREEN_RECORDER_SUPPORT start
	java.lang.Process mProcess = null;
	int mProcessPid = -1;
	String mPidString;
	private ScreenRecorderBroadcastReceiver mScreenRecorderReceiver = null;  
	private class ScreenRecorderBroadcastReceiver extends BroadcastReceiver {
		@Override
		public void onReceive(Context context, Intent intent) {
			//if(isScreenRecorderAppExist()){
				if ("android.fineos.screenrecorder.action.START_RECORD".equals(intent.getAction())) {
					startRecord();
				}else if("android.fineos.screenrecorder.action.STOP_RECORD".equals(intent.getAction())){
					stopRecord();
				}
			//}
		}
	...
	private void registerScreenRecorderBroadcast() {
		IntentFilter filter = new IntentFilter();
		filter = new IntentFilter();
		filter.addAction("android.fineos.screenrecorder.action.START_RECORD");
		filter.addAction("android.fineos.screenrecorder.action.STOP_RECORD");
		mScreenRecorderReceiver = new ScreenRecorderBroadcastReceiver();
		mContext.registerReceiver(mScreenRecorderReceiver, filter);
	}
	
	public void startRecord(){
		try{
		//	mProcess = Runtime.getRuntime().exec("screenrecord  --time-limit 10 /sdcard/demo.mp4", null, null);
		    //fineos start
			//mProcess = Runtime.getRuntime().exec("screenrecord  /sdcard/demo.mp4", null, null);
			SimpleDateFormat format = new SimpleDateFormat("yyyyMMddHHmmss");
			String screenVideoName = format.format(new Date());
			String startScreenRecordCmd = "screenrecord /sdcard/screenrecord_" + screenVideoName + ".mp4";
			mProcess = Runtime.getRuntime().exec(startScreenRecordCmd, null, null);
			//fineos end
			getPidFromProcessString();
	...
	public void stopRecord(){
		if(mProcess != null){
			mProcess = null;
			try{
				String temp = "kill -SIGINT " +mPidString;
				Runtime.getRuntime().exec(temp, null, null);
			}catch(IOException e){
				e.printStackTrace();
			} 
		}
	...
	public void getPidFromProcessString(){
		String processString = mProcess.toString();
		int firstpos = processString.lastIndexOf('=') + 1;
		int endpos = processString.lastIndexOf(']');
		mPidString = processString.substring(firstpos, endpos);
		mProcessPid = Integer.parseInt(mPidString);
	}

	public boolean isScreenRecorderAppExist(){
		Intent intent = new Intent();
		intent.setComponent(new ComponentName("com.android.screenrecorder", "com.android.screenrecorder.StartRecordActivity"));
