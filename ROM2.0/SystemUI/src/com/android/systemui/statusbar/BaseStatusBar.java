...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    //fineos STATUSBAR_ICON_COLOR_SUPPORT start
    protected static final int MSG_UPDATE_ICON_COLOR = 3039;
    //fineos STATUSBAR_ICON_COLOR_SUPPORT end
    ...
    protected void applyColorsAndBackgrounds(StatusBarNotification sbn,
        ...
        if (entry.icon != null) {
            ...
            if (entry.targetSdk >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                entry.icon.setColorFilter(PhoneStatusBar.getIconColor());
            } else {
                //entry.icon.setColorFilter(PhoneStatusBar.getIconColor());
    ...
    protected void handleVisibleToUserChanged(boolean visibleToUser) {
        try {
		    /// xuqian modified begin , turn on leds on StatusBarState.SHADE_LOCKED mode
            if (visibleToUser) {
                ...
                boolean clearNotificationEffects = false ;
				if(fineUtilities.useFineOSlideKeyguard()){
				
				   clearNotificationEffects = (mState == StatusBarState.SHADE);
				
				}else{
				
				  clearNotificationEffects = (mState == StatusBarState.SHADE || mState == StatusBarState.SHADE_LOCKED);
				
				}
				
                        
		 /// xuqian modified end  , turn on leds on StatusBarState.SHADE_LOCKED mode	
    ...
    protected void updateRowStates() {
	   ...
	   boolean keyguardShowing = mStatusBarKeyguardViewManager.isShowing();               
...
///FineOS Notification start	
				...
				}else
				{
                 ...
                 ///FineOS Notification (View.VISIBLE)
///FineOS Notification end								
                if (wasGone) {
...
///FineOS Notification starts	
        	
		 if(fineUtilities.useFineOSlideKeyguard()){
		 	mKeyguardIconOverflowContainer.setVisibility(View.GONE);///FineOS Notification (View.VISIBLE)		 	
		 }else{					
...
///FineOS Notification starts
        if(fineUtilities.useFineOSlideKeyguard()){
		if(visibleNotifications>=1){
			boolean IskeguardForDismiss = mStatusBarKeyguardViewManager.isShowing();               	
			if(IskeguardForDismiss)
			{
			mDismissView.SetDismissButtonVisible(View.INVISIBLE);
			}else
			{
			mDismissView.SetDismissButtonVisible(View.VISIBLE);
			}
		}
	}
///FineOS Notification end			
