 ...
 * @Description:获取农历日历类{modify for [HQ_BOWAY_KEYGUARD]}
 * @author:BingWu.Lee
 * @date:2015-6-10
 */
public class LunarCalendar {
    ...
    //wangxiaoyang mark for TraditionalFestival
    ...
    private String getFestival(int lunarMonth, int lunarDay) {
        ...
        if (lunarMonth == 6 && lunarDay == 1)
            return getString(R.string.Communist_day);
    ...
    private String getLunarSolarTerms(int year) {
        int yy = 0;
        ...
        for (int i = 0; i < lunar_century_C.length; i++) {
            ...
            if (i < 4) {
                ...
                day = (int) Math
                        .floor((yy * D_solar_terms + lunar_century_C[i])
                                - (yy / 4));
                /*Add 20130726 Spreadst of 193040 2016 xiaoshu is wrong start*/
                if ((year == 2016 || year == 1925) && i == 12) {
                    day = day + 1;
                }
                /*Add 20130726 Spreadst of 193040 2016 xiaoshu is wrong end*/
            }
            ...
            if (solarMonth == 3 && solarDay == day && i == 6) {
		        if(true){/* HQ00405450 lizhe*/
		  	        return getString(R.string.tomb_day) ;
		        } else {
		  	        return getString(R.string.terms6) + "/" + getString(R.string.tomb_day);
                }
            }
    ...
    private String getLunarYearString(int num) {
        String[] stemNames = new String[10];
        stemNames[0] = getString(R.string.jia);
        stemNames[1] = getString(R.string.yi);
        stemNames[2] = getString(R.string.bing);
        stemNames[3] = getString(R.string.ding);
        stemNames[4] = getString(R.string.wutian);
        stemNames[5] = getString(R.string.ji);
        stemNames[6] = getString(R.string.geng);
        stemNames[7] = getString(R.string.xin);
        stemNames[8] = getString(R.string.ren);
        stemNames[9] = getString(R.string.gui);

        String[] branchNames = new String[12];
        branchNames[0] = getString(R.string.zi);
        if (LunarCalendarConvertUtil.isLunarSetting()) {
            branchNames[1] = getString(R.string.choufan);
        } else {
            branchNames[1] = getString(R.string.chou);
        }
        branchNames[2] = getString(R.string.yin);
        branchNames[3] = getString(R.string.mao);
        branchNames[4] = getString(R.string.chen);
        branchNames[5] = getString(R.string.si);
        branchNames[6] = getString(R.string.wudi);
        branchNames[7] = getString(R.string.wei);
        branchNames[8] = getString(R.string.shen);
        branchNames[9] = getString(R.string.you);
        branchNames[10] = getString(R.string.xu);
        branchNames[11] = getString(R.string.hai);

        return (stemNames[num % 10] + branchNames[num % 12]);
    }
    ...
    public String animalsYear(int year) {
        String[] year_of_birth = new String[12];
        year_of_birth[0] = getString(R.string.animals0);
        year_of_birth[1] = getString(R.string.animals1);
        year_of_birth[2] = getString(R.string.animals2);
        year_of_birth[3] = getString(R.string.animals3);
...
/* Add 20121120 Spreadst of calendar_cmcc_7 calendar_cmcc_8 , Calendar display Chinese festival end*/
