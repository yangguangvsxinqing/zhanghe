...
/**
 * @ClassName:LunarCalendarConvertUtil
 * @Description:农历工具类{modify for [HQ_BOWAY_KEYGUARD]}
 * @author:BingWu.Lee
 * @date:2015-6-10
 */
public class LunarCalendarConvertUtil {
    ...
    public static void parseLunarCalendar(int year, int month, int day,
        ...
        //modify by chenkai, HQ00413487, 【Pre-OT1】【日历】日历中1991年9月15日之前对应的农历全部显示错误 begin
        int offsetDayNum;
        if (year == 1991 && (month > 3 && month < 8) 
        		|| (month == 3 && day > 15) 
        		|| (month == 8 && day < 16)){
            offsetDayNum = (int) ((presentDate.getTime() - baseDayTime + 3600000L) / 86400000L);
        } else {
            offsetDayNum = (int) ((presentDate.getTime() - baseDayTime) / 86400000L);
        }
       //modify by chenkai, HQ00413487, 【Pre-OT1】【日历】日历中1991年9月15日之前对应的农历全部显示错误 end
