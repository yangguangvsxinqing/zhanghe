...
final class GsonToMiniGsonTypeAdapterFactory implements TypeAdapter.Factory {
	...
	public <T> TypeAdapter<T> create(final MiniGson context, final TypeToken<T> typeToken) {
		...
		// guaranteed to match typeOfT
		final JsonSerializer<T> serializer = (JsonSerializer<T>) serializers.getHandlerFor(type, false);
		@SuppressWarnings("unchecked")
		// guaranteed to match typeOfT
		final JsonDeserializer<T> deserializer = (JsonDeserializer<T>) deserializers.getHandlerFor(type, false);
		...
		return new TypeAdapter<T>() {
			...
			private TypeAdapter<T> delegate() {
				...
				return d != null ? d : (delegate = context.getNextAdapter(GsonToMiniGsonTypeAdapterFactory.this, typeToken));
			}
		};
	}
}
