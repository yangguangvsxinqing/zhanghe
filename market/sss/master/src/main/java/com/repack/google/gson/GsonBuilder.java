...
public final class GsonBuilder {
	...
	private final List<TypeAdapter.Factory> typeAdapterFactories = new ArrayList<TypeAdapter.Factory>();
	 ...
	 * third-party sites through script sourcing. See <a
	 * href="http://code.google.com/p/google-gson/issues/detail?id=42">Gson
	 ...
	 * {@link com.repack.google.gson.annotations.Expose} annotation.
	 * 
	 * @return a reference to this {@code GsonBuilder} object to fulfill the
	 *         "Builder" pattern
	 */
	 ...
	 * {@code Point} class, but rather the default Gson serialization is
	 * applied. In this case, some {@code new Point(2,3)} would serialize as
	 * {@code {"x":2,"y":5}}.
	 * 
	 * <p>
	 ...
	 * combines the registration of an {@link InstanceCreator},
	 * {@link JsonSerializer}, and a {@link JsonDeserializer}. It is best used
	 * when a single object {@code typeAdapter} implements all the required
	 * interfaces for custom serialization with Gson. If an instance creator,
	 * serializer or deserializer was previously registered for the specified
	 * {@code type}, it is overwritten.
	 * 
	 * @param type
	 ...
	 * type corresponding to a raw type.
	 * 
	 * @param <T>
	 ...
	 * for different generic types corresponding to a raw type.
	 * 
	 * @param <T>
	 ...
	 * deserializers for different generic types corresponding to a raw type.
	 * 
	 * @param <T>
	 ...
	 * inheritance type hierarchy. This method combines the registration of an
	 * {@link InstanceCreator}, {@link JsonSerializer}, and a
	 ...
	 * serialization with Gson. If an instance creator, serializer or
	 * deserializer was previously registered for the specified type hierarchy,
	 * it is overwritten. If an instance creator, serializer or deserializer is
	...
	private <T> GsonBuilder registerDeserializerForTypeHierarchy(Class<?> classOfT, JsonDeserializer<T> deserializer, boolean isSystem) {
		deserializers.registerForTypeHierarchy(classOfT, new JsonDeserializerExceptionWrapper<T>(deserializer), isSystem);
		return this;
	}
	 ...
	 * >Javascript specification</a> (see section 4.3.20, 4.3.22, 4.3.23) allows
	 * these values as valid Javascript values. Moreover, most JavaScript
	 * engines will accept these special values in JSON without problem. So, at
	 * a practical level, it makes sense to accept these values as valid JSON
	 * even though JSON specification disallows them.
	 * 
	 * <p>
	 ...
	 */
	public Gson create() {
		List<ExclusionStrategy> deserializationStrategies = new LinkedList<ExclusionStrategy>(deserializeExclusionStrategies);
		List<ExclusionStrategy> serializationStrategies = new LinkedList<ExclusionStrategy>(serializeExclusionStrategies);
		...
		return new Gson(new DisjunctionExclusionStrategy(deserializationStrategies), new DisjunctionExclusionStrategy(serializationStrategies), fieldNamingPolicy, instanceCreators.copyOf()
				.makeUnmodifiable(), serializeNulls, serializers.copyOf().makeUnmodifiable(), deserializers.copyOf().makeUnmodifiable(), complexMapKeySerialization, generateNonExecutableJson,
				escapeHtmlChars, prettyPrinting, serializeSpecialFloatingPointValues, longSerializationPolicy, typeAdapterFactories);
	}

	private static void addTypeAdaptersForDate(String datePattern, int dateStyle, int timeStyle, ParameterizedTypeHandlerMap<JsonSerializer<?>> serializers,
