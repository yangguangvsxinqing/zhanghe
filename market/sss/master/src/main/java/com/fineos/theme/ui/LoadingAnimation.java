...
public class LoadingAnimation extends AnimationDrawable {
	...
	/*
	 * style - Rectangle size color1 - Focus Rectangle color id color2 - Normal
	 * Rectangle color id duration - time to display each frame in
	 * AnimationDrawable
	 */
	public LoadingAnimation(Context context, int style, int color1, int color2, int duration) {
	...
	private void initAnimation() {
		...
		for (int i = 0; i < ANI_NUMBER; i++) {
			Bitmap bitmap = Bitmap.createBitmap((int) nWidth, (int) nHeight, Bitmap.Config.ARGB_4444);
			...
			if (bitmap != null) {
				...
				for (int j = 0; j < ANI_NUMBER; j++) {
					...
					if (j == i) {
						// Focus Rectangle
						canvas.drawRect((x - nSpaceFocus), (y - nSpaceFocus), (x + nSpaceFocus), (y + nSpaceFocus), paint1);
					} else {
						// Normal Rectangle
						canvas.drawRect((x - nSpace), (y - nSpace), (x + nSpace), (y + nSpace), paint2);
					}
				}
				canvas.save();
