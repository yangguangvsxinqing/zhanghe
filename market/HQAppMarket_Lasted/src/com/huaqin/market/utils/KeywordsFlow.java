...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	public static final int TEXT_SIZE_MAX = 27;
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
			...
			for (int i = 0; i < size; i++) {
				...
				// 随机颜色
				int ranColor = 0xff000000 | (random.nextInt(0x00fff000-0x000fff)+0x00000fff);
				// 随机位置，糙值
				int xy[] = randomXY(random, listX, listY, xItem);
				// 随机字体大小
				int txtSize = 20;//TEXT_SIZE_MIN  + random.nextInt(TEXT_SIZE_MAX - TEXT_SIZE_MIN + 1);
				// 实例化TextView
